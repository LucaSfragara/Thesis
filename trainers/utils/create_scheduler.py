...
def create_scheduler(
    ...
    if scheduler_name == 'reduce_lr':
        ...
        #T_max_epochs = len(train_loader)#cosine_config.get('T_max', 60)
        T_max_steps = len(train_loader)#_epochs_to_steps(T_max_epochs, train_loader, gradient_accumulation_steps)
        
        print("├── Cosine Annealing Settings:")
        print(f"│   ├── T_max: {T_max_steps} steps")
        print(f"│   └── Min LR: {cosine_config.get('eta_min', 0.0001)}")

        base_scheduler = lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=len(train_loader), #T_max_steps,  # Convert to steps
...
def create_warmup_scheduler(
    ...
    """
    warmup_steps_fraction = warmup_config.get('warmup_steps_fraction', 5)
    ...
    warmup_steps = int(len(train_loader) * warmup_steps_fraction)
...
def plot_lr_schedule(
    ...
    if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
        ...
        total_steps = len(train_loader)#_epochs_to_steps(num_epochs, train_loader, gradient_accumulation_steps)
