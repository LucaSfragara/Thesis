...
class SequenceGenerator:
    ...
    def __init__(
        ...
        self.device = device
        self.config = config
        self.eos_id = config["data"]["eos_token"]
        self.sos_id = config["data"]["sos_token"]
    ...
    def generate_greedy(
        ...
        batch_size, current_sequence_length = x.shape
        
        finished = torch.zeros(batch_size, device = x.device).to(torch.bool)
        scores = torch.zeros(batch_size, device = x.device)

        for t in range(self.max_length - current_sequence_length):
            
            logits = self.score_fn(x) #(batch_size, num_classes)
            #logits = self._apply_repeat_penalty(logits, x, penalty = repeat_penalty)
            logits = logits / temperature
            
            log_probs = nn.functional.log_softmax(logits, dim = -1) #(batch_size, num_classes)
            next_tokens = torch.argmax(log_probs, dim = -1) #(batch_size,)
            token_scores = log_probs[torch.arange(batch_size), next_tokens]

            next_tokens =  next_tokens.unsqueeze(1) # (batch_size, 1)
            
            x = torch.cat((x, next_tokens), dim = 1)
            
            token_scores = token_scores * (~finished)
            
            scores += token_scores
            
            finished |= (next_tokens.squeeze(1) == self.eos_id)

        
            if finished.all():
                break
            
        #Remove all content after EOS token
    ...
    @staticmethod
    def post_process_sequence(seq: torch.Tensor, eos_token:int) -> torch.Tensor:
        """
        ...
        if seq.dim() == 1:
            eos_indices = (seq == eos_token).nonzero()
        ...
        eos_mask = seq == eos_token  # (batch_size, sequence_length)
